...
5. Важно! Для работы с динамическими библиотеками INDIGO необходимо добавить в переменную окружения `LD_LIBRARY_PATH` папку indigo/build/lib/. Текущее временное решение — хранить эту переменную в файле .env. Нужно добавить последнюю строку из выдачи `make install` туда в виде:

```bash
# in .env
LD_LIBRARY_PATH="/path/to/indigo/libs"
```
